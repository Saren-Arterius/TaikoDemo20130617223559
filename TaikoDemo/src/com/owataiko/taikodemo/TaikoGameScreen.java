...
public class TaikoGameScreen extends EbScreen<TaikoGame> implements Screen {
    ...
    public void onScreenRender(float aDelta) {
        ...
        final long bgmPos = System.currentTimeMillis() - iStartTime;
        if (iPlaying) {
            int scanIdxStart = (int) (Math.floor((bgmPos - GoodbyeChalon.start_time * 1000 - SCREEN_PERIOD / 2)
                    / (GoodbyeChalon.unit_time * 1000)));
            int scanIdxEnd = (int) (Math.floor((bgmPos - GoodbyeChalon.start_time * 1000 + 3 * SCREEN_PERIOD / 2)
                    / (GoodbyeChalon.unit_time * 1000)));
            scanIdxStart = Math.max(0, scanIdxStart);
            scanIdxEnd = Math.min(scanIdxEnd, GoodbyeChalon.note.length);

            for (int idx = scanIdxStart; idx < scanIdxEnd; ++idx) {
                Texture tx = null;
                if (GoodbyeChalon.note[idx] == 1) {
                    tx = mDonSmallTexture;
                } else if (GoodbyeChalon.note[idx] == 2) {
                    tx = mKatSmallTexture;
                } else if (GoodbyeChalon.note[idx] == 3) {
                    tx = mDonBigTexture;
                } else if (GoodbyeChalon.note[idx] == 4) {
                    tx = mKatBigTexture;
                } else if (GoodbyeChalon.note[idx] == 5) {
                    tx = mLongSmallTexture;
                } else if (GoodbyeChalon.note[idx] == 55) {
                    tx = mLongSmallTexture;
                } else {
                    continue;
                }
                long time = (long) ((idx * GoodbyeChalon.unit_time + GoodbyeChalon.start_time) * 1000);
                time -= bgmPos;
        ...
        final Texture tx2 = iSupportLowLatency ? mKatBigTexture : mDonBigTexture;
        tx2.bind();
        mUnitRectMesh.render(GL10.GL_TRIANGLE_STRIP);
    ...
    protected void onScreenLoadMember() {
        ...
        mNoteAreaBackgroundMesh = MeshUtils.newBoxMesh(0, mScreenHeight, mScreenWidth, mScreenHeight - iNoteAreaHeight);

        mLeftKatMesh = MeshUtils.newTriangleMesh(0, iPadSize, iPadSize, iPadSize, 0, 0);
        mRightKatMesh = MeshUtils.newTriangleMesh(mScreenWidth, iPadSize, mScreenWidth - iPadSize, iPadSize,
                mScreenWidth, 0);

        mLeftDonMesh = MeshUtils.newTriangleMesh(iPadSize, iPadSize, iPadSize, 0, 0, 0);
        mRightDonMesh = MeshUtils.newTriangleMesh(mScreenWidth - iPadSize, iPadSize, mScreenWidth - iPadSize, 0,
                mScreenWidth, 0);
        ...
        mLongSmallTexture = new Texture(Gdx.files.internal("data/long0_small.png"));
