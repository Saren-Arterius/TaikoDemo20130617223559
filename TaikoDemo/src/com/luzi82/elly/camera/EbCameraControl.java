...
public class EbCameraControl {
	...
	public static final float DIV_LN_SMOOTH_REDUCE = (float) (1 / Math
			.log(SMOOTH_REDUCE));
	...
	public void update(float aDelta) {
		...
		if (touchCount > 0) {
			...
			if (mTouchCountChange) {
				...
				if (touchCount > 1) {
					...
					float newCameraWX = mCameraCalc.screenWorldToCameraWX(
							touchSXAvg, mTouchStartWXAvg);
					float newCameraWY = mCameraCalc.screenWorldToCameraWY(
							touchSYAvg, mTouchStartWYAvg);
					mCameraCalc.xyMove(newCameraWX, newCameraWY, aDelta);
				}
				mTouchCountChange = false;
			} else if (mTouchChange) {
				if (touchCount > 1) {
					float newZoom = mTouchStartCameraZoom * mTouchStartSDiff
							/ touchSDiff;
			...
			float mouseBX = mCameraCalc.screenToWorldX(mMouseOverSX);
			float mouseBY = mCameraCalc.screenToWorldY(mMouseOverSY);

			mCameraCalc.mCameraCalcZoomD -= mMouseScrolled * Const.PHI;
			mCameraCalc.smoothZoom(aDelta, reduce, intReduce);

			float newCameraWX = mCameraCalc.screenWorldToCameraWX(mMouseOverSX,
					mouseBX);
			float newCameraWY = mCameraCalc.screenWorldToCameraWY(mMouseOverSY,
					mouseBY);
			mCameraCalc.xyMove(newCameraWX, newCameraWY, aDelta);
			mMouseScrolled = 0;
		} else {
		...
		if (mTouchCountChange && (touchCount == 0)
				&& (mCameraCalc.mLockTime > 0)) {
	...
	public void touchDragged(int aSX, int aSY, int aPointer, long aTime) {
		...
		float diff = EbCameraCalc.diff(aSX, aSY, mTouchStartSX[aPointer],
