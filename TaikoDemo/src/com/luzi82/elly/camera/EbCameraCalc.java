...
public class EbCameraCalc {
    ...
    public void updateLock(float aReduce) {
        ...
        if (mLockTime < 0) {
            ...
            clr = EbCameraCalc.calcLock(iCameraRealWX, iCameraCalcWX, mLockWXDiff, aReduce);
            iCameraRealWX = clr.mValue;
            mLockWXDiff = clr.mDiff;

            clr = EbCameraCalc.calcLock(iCameraRealWY, iCameraCalcWY, mLockWYDiff, aReduce);
            iCameraRealWY = clr.mValue;
            mLockWYDiff = clr.mDiff;

            clr = EbCameraCalc.calcLock((float) Math.log(iCameraRealZoom), (float) Math.log(iCameraCalcZoom),
                    mLockZoomDiff, aReduce);
            ...
            mLockZoomDiff = Math.abs((float) Math.log(iCameraRealZoom / iCameraCalcZoom));
        }
    }
    ...
    public float screenToWorldY(float aScreenY) {
        return iCameraCalcWY + (iCameraCalcZoom * mViewPortH * (1 - (aScreenY / mScreenH) - 0.5f));
    }

    public float screenToWorldRealX(float aScreenX) {
        return iCameraRealWX + (iCameraRealZoom * mViewPortW * (aScreenX / mScreenW - 0.5f));
    }

    public float screenToWorldRealY(float aScreenY) {
        return iCameraRealWY + (iCameraRealZoom * mViewPortH * (1 - (aScreenY / mScreenH) - 0.5f));
    }

    public float screenWorldToCameraWX(float aScreenX, float aWorldX) {
        return (iCameraCalcZoom * mViewPortW) * (0.5f - aScreenX / mScreenW) + aWorldX;
    }

    public float screenWorldToCameraWY(float aScreenY, float aWorldY) {
        return (iCameraCalcZoom * mViewPortH) * (0.5f + aScreenY / mScreenH - 1) + aWorldY;
    }
    ...
    public void onScreenResize(int aScreenWidth, int aScreenHeight) {
        ...
        mZoomMax = Math.max(mWorldH / mViewPortH, mWorldW / mViewPortW) * Const.PHI;
    ...
    public void smoothZoom(float aDelta, float aReduce, float aIntReduce) {
        ...
        logCameraZoom = EbCameraCalc.smooth(aDelta, aReduce, aIntReduce, logCameraZoom, mCameraCalcZoomD, mLogZoomMin,
                mLogZoomMax);
    ...
    public void smoothXY(float aDelta, float aReduce, float aIntReduce) {
        ...
        iCameraCalcWX = EbCameraCalc.smooth(aDelta, aReduce, aIntReduce, iCameraCalcWX, mCameraCalcWXD, 0, mWorldW);
        iCameraCalcWY = EbCameraCalc.smooth(aDelta, aReduce, aIntReduce, iCameraCalcWY, mCameraCalcWYD, 0, mWorldH);
        mCameraCalcWXD *= aReduce;
        mCameraCalcWYD *= aReduce;
    }

    private static float smooth(float aDelta, float aReduce, float aIntReduce, float aS0, float aV, float aMin,
            float aMax) {
        if ((aMin <= aS0) && (aS0 <= aMax)) {
            final float s1 = aS0 + aV * aIntReduce;
            // TODO mid border
            return s1;
        } else {
            final float border = (aS0 < aMin) ? aMin : aMax;
            final float dm = aV * aIntReduce;
            final float db = (aReduce - 1) * (2 * aS0 - 2 * border + dm) / 2;
            final float s1 = aS0 + dm + db;
            // TODO mid border
            return s1;
        }
    ...
    public static float diff(float aX0, float aY0, float aX1, float aY1) {
        final float dx = aX1 - aX0;
        final float dy = aY1 - aY0;
