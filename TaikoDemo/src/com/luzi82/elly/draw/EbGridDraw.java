...
public class EbGridDraw extends EbDraw {
    ...
    public void drawStatic(GL10 aGl, EbLogicScreen<?, ?> aScreen) {
        ...
        final EbCameraCalc mCameraCalc = aScreen.mCameraCalc;
        final float worldW = aScreen.mLogic.mWorldW;
        final float worldH = aScreen.mLogic.mWorldH;
        ...
        while (factor < mCameraCalc.mZoomMax) {
            ...
            if (a > 0) {
                // if (a > 1)
                // a = 1f;
                a *= EbGridDraw.LINE_ALPHA;
                for (final int ai: LINE_MESH_HA) {
                    mLineMeshHF[ai] = a;
                }
                ...
                min = (int) Math.floor(Math.max(0, mCameraCalc.viewWorldYMin()));
                min = (min / factor) * factor;
                max = (int) Math.ceil(Math.min(worldH, mCameraCalc.viewWorldYMax()));
                ...
                min = (int) Math.floor(Math.max(0, mCameraCalc.viewWorldXMin()));
                min = (min / factor) * factor;
                max = (int) Math.ceil(Math.min(worldW, mCameraCalc.viewWorldXMax()));
                aGl.glPushMatrix();
    ...
    public void alloc() {
        ...
        1f, 0f, 0f, 0f, 0f, 0f, EbGridDraw.LINE_ALPHA,//
                0f, 0f, 0f, 0f, 0f, 0f, EbGridDraw.LINE_ALPHA,//
                1f, EbGridDraw.LINE_WIDTH, 0f, 0f, 0f, 0f, 0f,//
                0f, EbGridDraw.LINE_WIDTH, 0f, 0f, 0f, 0f, 0f,//
